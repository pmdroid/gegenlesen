# Meister: Hosted PR Review Service
...
| **Author** | TBD |
| **Date** | 2026-08-17 |
| **Audience** | Senior engineers implementing Meister from a greenfield repo |
| **Workspace** | `/Users/pascal/work/meister` |
| **Related products** | OpenCode (`anomalyco/opencode`, https://opencode.ai) — *not* the archived Go CLI `opencode-ai/opencode` |
| **Canonical copy** | [`docs/meister-pr-review-service.md`](meister-pr-review-service.md) (this file). Summary at repo root [`README.md`](../README.md). |
...
AI PR reviewers (CodeRabbit, Greptile, Cursor Bugbot, Copilot review, OpenCode’s own GitHub Action) share a common pipeline shape: **scope the diff → apply path-filtered rules → agent reads the tree → emit line-anchored comments**. The ones that stay usable add two more things Meister will treat as first-class:
...
2. **A second pass that is allowed to say “no”.** Independent review (CodeRabbit’s “writer ≠ reviewer” argument; Bugbot’s downvote-to-rule loop) is how you keep precision from collapsing. Meister’s judge is that second pass. It does not re-review the world. It only sees candidate findings plus the evidence the reviewer cited.

Meister is not a GitHub App in v1. The interface is **manual upload of a change**. That is deliberate: it unblocks local / air-gapped / pre-push review, and it forces the upload + incremental-diff contract to be real rather than hidden behind a webhook.
...
- Replacing CI. Meister does not gate merges and does not publish check runs.
...
| K2 | **Persistence: SQLite via GRDB.swift + FTS5, plus a filesystem blob store** | Single tenant, one host. GRDB has real migrations and FTS5 (BM25). Wrap the database in an actor. No Postgres, no Redis in v1. Config lives in `config/meister.json` / env — **no `settings` table**. |
...
| K6 | **Findings contract is a file, not the event stream** | Each reviewer writes `/workspace/.meister/findings-model_a.json` or `findings-model_b.json` (same JSON Schema as `findings.agent.json`). Parallel runs must not share `findings.json`. |
...
| K8 | **Concurrency = 1 Meister job; 2 reviewer containers during `reviewing`** | Reviewer A and B run **at the same time**. Judge is one container after both complete. One job in the queue at a time. Each container still has its own CPU/memory cap. |
...
| K13 | **No auth. Bind `127.0.0.1`. Refuse `0.0.0.0` unless `MEISTER_ALLOW_REMOTE=1`.** | Single tenant. Security work goes into extract sandboxing, Docker isolation, and secret hygiene — not a login page. |
...
| K16 | **Memory queue is not durable; boot reconciles** | On process start: `docker rm -f` every `meister-*` container; any `jobs` row not in `{succeeded,failed,cancelled}` becomes `failed` with `error_message=process_restarted`, except rows that are still `queued` **and** `started_at IS NULL` — those are re-`push`ed. |
...
| K19 | **v1 egress = user-defined Docker bridge. Agent HTTP is published to host loopback only.** | Container outbound on `meister-egress`. `opencode serve` is mapped `-p 127.0.0.1:${ephemeral}:4096` — never `0.0.0.0` on the host. The React app does not call that port. |
...
| K25 | **Store embeddings. Vector-search context at review time. Learn a living architecture card.** | After identify (and again after learn), chunk the tree + user notes + accepted architecture. Embed with the configured embedding model. At review time retrieve top-K chunks into `.meister/context.md`. Architecture is a structured note the operator can edit. |
| K26 | **User-provided context in the UI. Suggested learnings inbox after every run.** | `/context` is a notebook the operator writes (architecture, logger choice, “never touch Payments”). `/learnings` lists drafts Meister produced from a job or corpus. Accept → context or rule. Dismiss → gone. Nothing auto-applies. |
| K27 | **Split: CLI runs reviews; browser manages knowledge.** | `Sources/MeisterCLI` talks to the local API. The React app does not start Docker or pack a repo. |
...
flowchart LR
  ...
  subgraph Host["Host process: MeisterAPI"]
    HTTP[Vapor HTTP]
...
Deterministic hits are ordinary finding rows. They **never** fail the job and they **never** skip the reviewer by themselves (the reviewer is skipped only for `MEISTER_SKIP_AGENT`, or incremental-with-no-new-work — see the state machine).

The host never writes a model API key into a review artifact. Keys are injected as container env **only** for reviewer / judge / miner phases and exist only in process memory on the host (`config/meister.json` is gitignored; `meister.example.json` is the template).

### Repo layout (target)

```
/Users/pascal/work/meister/
  Package.swift                      # // swift-tools-version: 6.0
  Sources/
    CLibArchive/                     # C shim around libarchive
      include/meister_archive.h
      meister_archive.c
    MeisterAPI/
  ...
  docker/opencode-runner/
    ...
    entrypoint.sh
  config/
    meister.example.json
  scripts/
    dev.sh
    pack-repo.sh
    build-runner.sh                  # pins tag meister/opencode-runner:0.1.0
  var/
  .gitignore
```

`Package.swift` products: executable `MeisterAPI` depending on libraries `MeisterCore`, `MeisterDeterministic`, `MeisterAgent`, `MeisterMiner`. `MeisterCore` depends on `CLibArchive`.
...
| `apple/swift-argument-parser` | `MeisterAPI serve --bind --port --data-dir` |
...
- `MEISTER_SKIP_AGENT=1` (CI) and incremental-with-no-new-work both take `deterministic → succeeded`. Carry-forward findings are persisted first.
...
sequenceDiagram
  ...
  participant CLI as Meister CLI
...
Meister drives OpenCode through **`opencode serve`**, not ACP and not `opencode run` (run is fallback only).
...
4. `POST /session` `{ "title": "meister-review-${JOB_ID}" }` → `session.id`.
5. `POST /session/:id/message` with `agent`, `model: { providerID, modelID }`, and `parts`:
   - `{ "type": "text", "text": "<contents of prompt.md>" }`
   - `{ "type": "file", "path": "/workspace/.meister/rules.json" }` (and `diff.patch` / `judge-input.json` / `prompt-judge.md` as needed)
...
7. **Start both reviewers at once.** Two ports, two passwords, two containers (`meister-review-${JOB_ID}-a` and `-b`). Each prompt says: write **only** `.meister/findings-model_a.json` or `.meister/findings-model_b.json` (never the shared `findings.json`).
...
`OpenCodeHTTPClient` lives in `MeisterAgent`. Do not vendor `@opencode-ai/sdk` (TypeScript). Routes we call:
...
`MeisterAgent.Quarantine.run(workspace:)` therefore does two things:

**1. Copy (audit + citation).** Copy any of the following into `.meister/quarantine/<original relative path>` if they exist at the workspace root or any ancestor inside the workspace:
...
| --- | --- |
| `opencode.json` | `opencode.json.meister-disabled` |
| `opencode.jsonc` | `opencode.jsonc.meister-disabled` |
| `.opencode/` | `.opencode.meister-disabled/` |
...
Identification (`git diff` / `.meister/diff.patch`) runs on the **pre-quarantine** tree, so the patch still names `opencode.json` / `.opencode/…`. Those paths must remain **citable**:
...
if file_path is "opencode.json" or "opencode.jsonc"
   or file_path is under ".opencode/":
    q = ".meister/quarantine/" + file_path
    if q exists: return q
if file_path exists: return file_path
return missing
```

A finding that cites `opencode.json:12` is **not** discarded just because the live name is `opencode.json.meister-disabled`. The parser does **not** treat the renamed live file as the cited path (the snippet lives in the quarantine copy).
...
`RUNNER_CONFIG` is the **whole** `docker/opencode-runner/` config tree (`opencode.json` + `agents/`), **not** the `agents/` folder alone. Mounted at `/home/meister/.config/opencode` so `agents/reviewer.md` loads as the `reviewer` agent.
...
```bash
# Host-side (MeisterAgent.OpenCodeInvocation.review)
...
docker run --rm \
  ...
  --tmpfs /home/meister/.local:rw,nosuid,nodev,uid=1000,gid=1000,size=256m \
  --tmpfs /home/meister/.config/opencode-state:rw,nosuid,nodev,uid=1000,gid=1000,size=64m \
  --mount "type=bind,src=${WORKSPACE},dst=/workspace" \
  --mount "type=bind,src=${RUNNER_CONFIG},dst=/home/meister/.config/opencode,readonly" \
  --cpus="${MEISTER_DOCKER_CPUS:-2}" \
  --memory="${MEISTER_DOCKER_MEMORY:-4g}" \
  --pids-limit 256 \
  --cap-drop ALL \
  ...
  -e OPENCODE_CONFIG=/home/meister/.config/opencode/opencode.json \
  ...
  "meister/opencode-runner:${OPENCODE_IMAGE_TAG}" \
...
- Persist **both** names: `container_name_a` = `meister-review-${JOB_ID}-a`, `container_name_b` = `meister-review-${JOB_ID}-b`. After reviewers exit, `container_name` = `meister-judge-${JOB_ID}`. Cancel and boot reconcile `docker rm -f` every `meister-*`.

Timeout: `DockerRunner` owns a `Task` that calls `docker kill` at `MEISTER_AGENT_TIMEOUT_SEC` (default **900**) **per reviewer**. That same Task is the watchdog. Max captured stdout+stderr: **20 MiB**, then SIGKILL. After **both** reviewers: if new work existed and **neither** produced a valid findings file, the job **fails**. One valid file (even empty `{"findings":[]}`) is enough to continue. Zero surviving findings across both → skip judge.
...
1. Read `.meister/findings-model_a.json` and `.meister/findings-model_b.json` (copies of each agent file). A missing/invalid file for one slot is skipped. Both invalid + new work → job `failed`.
...
4. Copy the **unmodified** agent file to `.meister/agent-findings.json` and `var/blobs/findings/{id}-agent.json`.
5. Write **`.meister/judge-input.json`** (schema below) containing only candidates that will be judged (`phase=agent` and `command` JSONL). Each object has the **host** `id`, `evidence_ok`, and `actual_slice`.
...
9. Parse `.meister/judge.json`; run [Judge merge](#judge-merge-algorithm) keyed on host ULIDs.
...
Phase-specific env. **No** provider keys. **No** `meister-egress`. `--network none`. argv from the rule only.

```bash
docker run --rm \
  --name "meister-cmd-${JOB_ID}-${RULE_ID}" \
  --network none \
  --workdir /workspace \
  --user 1000:1000 \
  --read-only \
...
Same as reviewer (keys + `meister-egress` + `OPENCODE_CONFIG_CONTENT`) but `--agent miner`, workspace is a corpus staging dir, writes `.meister/mined-rules.json`.

#### Fallback if the CLI is slightly different

`OpenCodeInvocation` is a single struct. `Tests/MeisterAgentTests/OpenCodeCLIProbeTests.swift` runs `docker run … opencode --help` and `opencode run --help` against the pinned image and asserts the flags we need. If a flag is missing:

| Missing flag | Fallback |
| --- | --- |
| `--dir` | `docker run --workdir /workspace` and omit `--dir` |
| `--format json` | omit; rely on `.meister/findings.json` only |
...
# useradd -u 1000 -m meister
# curl -fsSL GitHub release asset for anomalyco/opencode @ ${OPENCODE_VERSION}
LABEL org.meister.opencode-version="${OPENCODE_VERSION}"
LABEL org.meister.runner-version="0.1.0"
# COPY opencode.json agents/ into /home/meister/.config/opencode/
# chown -R 1000:1000 /home/meister
ENTRYPOINT ["/usr/local/bin/entrypoint.sh"]
```

Image tag is a **semver of the runner**, not a date: `meister/opencode-runner:0.1.0`. `scripts/build-runner.sh` always retags that version; bump it when the Dockerfile or baked agents change. Never `:latest` in `meister.json`. Pin `OPENCODE_VERSION` (e.g. `1.1.25`) in the build script.

`meister-egress` is a user-defined bridge with **no published ports**. v1 does not install a custom iptables egress filter (OQ-1). Inbound to the container is impossible because nothing is published.
...
```json
{
  ...
  "$id": "https://meister.local/schemas/findings.agent.json",
  "type": "object",
...
`evidence_ok` is computed **before** the judge (handoff step 3): resolve `file_path` with `Workspace.resolveForRead` (so a citation of `opencode.json` reads `.meister/quarantine/opencode.json`), take lines `[start_line, end_line]` as `actual_slice`, test whether `normalize_ws(snippet)` is a substring of `normalize_ws(actual_slice)`. Both fields are written into `.meister/judge-input.json`. The judge never sees the raw agent file.
...
- `var/blobs/findings/{id}-agent.json` — unmodified agent `.meister/findings.json`
- `.meister/judge-input.json` and `var/blobs/findings/{id}-pre-judge.json` — host ULIDs + `evidence_ok` + `actual_slice`
...
1. `docker ps -a --filter name=meister- --format '{{.Names}}'` then `docker rm -f` each. This reaps orphans left by `docker run --rm` when the host was SIGKILL’d (the Swift `defer` did not run; containers may still hold keys in env).
2. SQL:
...
Retries: **0** for `meister.review`, `meister.mine`, `meister.gc`. Document in the `registerJob` call.
...
| --- | --- |
| Linux | **Required.** Failure fails the job (`identifying` never starts). uid 1000 must own the bind mount or the agent cannot write `.meister/findings.json`. |
...
| --- | --- | --- |
| `archive` | **always** | `.tar` or `.tar.gz` / `.tgz`. **Zip rejected.** Working tree. May contain `.git`, `.meister/history.bundle`, and/or `.meister/diff.patch` — the handler does **not** look inside the tar at POST time. |
| `patch` | **optional** | Unified diff (`diff --git`) **or** `git format-patch` mbox. Needed at HTTP **only** when `meta` also lacks both SHAs (see 422). Preferred `pack-repo.sh` uploads omit this part and embed `.meister/diff.patch` instead. |
...
| `422` | incremental parent missing / not `succeeded` / missing `base_sha`+`head_sha`+at least one `job_files` row; **`meta` lacks both `base_sha` and `head_sha` AND there is no `patch` part**. In-archive `.git` / bundle / `.meister/diff.patch` are **not** inspected here. `reviewer_model` in meta, if sent, is **ignored**. |
...
A `pack-repo.sh` tarball with empty `meta` SHAs and no `patch` part is therefore **202**, not 422. Identifying discovers `.meister/diff.patch` (and optional bundle / `.git`) after extract. If after extract there is still no history **and** no patch (multipart part or `.meister/diff.patch`), the **job** fails in `identifying` with `error_message=no_change_set`.
...
The script **exits non-zero** unless a usable `.meister/diff.patch` exists (empty is allowed only when `BASE == HEAD`) or a fetchable self-contained bundle was kept. Never `git bundle create … "$BASE..$HEAD"`.
...
#### Change-set identification (`MeisterCore.Git.ChangeSet`)
...
1. **Embedded pack-repo diff (preferred).** If `.meister/diff.patch` exists, copy it to `var/blobs/patches/{id}.patch`. Parse it for `job_files` (`old_path` on renames). Content SHA-256 of each head path from the **working tree** via CryptoKit. `base_sha` / `head_sha` from `meta`, else `.meister/base_sha` / `.meister/head_sha`, else `sha256(patch)` / `"noparent"`. This path does **not** need `.git`.
2. **History source** (used to refine SHAs, enable `git diff` against a parent, and incremental `parent.head_sha` lookups):
   ...
   - else `.meister/history.bundle` → `git init` (if needed) then **`git fetch .meister/history.bundle '+refs/*:refs/bundle/*'`** (or `git fetch .meister/history.bundle HEAD:refs/heads/bundle-head`). **Do not `git bundle unbundle`** — unbundle does not create refs, and an `A..B` bundle would fail here with `The bundle requires these refs`. Then `git checkout` of `meta.head_sha` or `.meister/head_sha` or `refs/bundle/heads/*`.
3. If history exists **and** step 1 did not already set the patch: `git diff --no-color --find-renames $base $head` → `var/blobs/patches/{id}.patch`. **No `--find-copies-harder`.** `git diff --name-status --find-renames` → `job_files`. Per-file content SHA-256 of head bytes via **CryptoKit** (never BSD `shasum`, never git blob SHA-1).
   - `head = meta.head_sha ?? contents(.meister/head_sha) ?? rev-parse(meta.head_ref ?? "HEAD")`
   - `base` = `meta.base_sha`; else `.meister/base_sha`; else `rev-parse(meta.base_ref)`; else `merge-base` of `main`/`master`/`trunk`/`origin/HEAD` and `head`; else `head^`; else empty tree `4b825dc642cb6eb9a060e54bf8d69288fbee4904`.
...
Semantic payload: `instruction` (required) + `few_shots[]` (optional excerpts). This is **guidance**, not a regex. Both reviewers receive it in `.meister/rules.json` and must apply it. Example (seed `rules/use-project-logger.yaml`):
...
**Embedding model** (`config/meister.json`):
...
Env: `MEISTER_EMBEDDING_MODEL`. The host calls the provider’s embeddings HTTP API (this is the one exception to “host never talks to a model”). Keys are the same provider keys already used for OpenCode. If the embedding call fails, review still runs: glob + FTS5 only, and a `job_events` warning.
...
| `architecture` | Accepted architecture card + per-module notes Meister drafted and the operator accepted. |
...
**Vector search:** brute-force cosine in the `Store` actor. Query = changed paths + `git diff` symbol tokens + job title. Return top `retrieve_k`. Also always include every `user` chunk marked `always_include=1`. Write the pack to `.meister/context.md` before either reviewer starts.
...
3. Ask a **short** OpenCode `miner` session (or skip if `MEISTER_SKIP_AGENT`) to draft an architecture card: layers, entrypoints, logging, forbidden areas. Output `.meister/architecture-draft.md`.
...
**Learnings inbox UI** (`/learnings`) — what Meister suggests after a run:
...
`MeisterCore.PathGlob` implements gitignore-style matching in-process (no extra package; Swift has no gitignore stdlib):
...
3. Render in rank order until `MEISTER_RULE_TOKEN_BUDGET` (default **6000** tokens, `chars/4`). Truncate few-shots first, then mined rules, never handwritten titles.
...
```
.meister/diff.patch              # from pack-repo or computed git diff
.meister/rules.json
.meister/files.json              # [{path, status, sha256, language, old_path}]
.meister/prompt.md
.meister/prompt-judge.md
.meister/context.md                  # retrieved chunks + architecture + user notes
.meister/parent-findings.json
.meister/findings.schema.json    # copy of schemas/findings.agent.json
.meister/judge.schema.json
# written later by the host, not PromptRenderer:
.meister/findings.json           # agent output
.meister/agent-findings.json     # copy of agent output
.meister/judge-input.json        # host ULIDs + evidence_ok + actual_slice
.meister/judge.json              # judge output
```
...
## Output
Write EXACTLY one JSON object to .meister/findings-{{slot}}.json matching
...
- Do not modify any file except .meister/findings-{{slot}}.json.
...
`config/meister.json` (path overridable by `MEISTER_CONFIG`):
...
Env overrides: `MEISTER_MODEL_A`, `MEISTER_MODEL_B`, `MEISTER_JUDGE_MODEL`, `MEISTER_DATA_DIR`, `MEISTER_BIND`, `MEISTER_PORT`, `MEISTER_ALLOW_REMOTE` (`0`/`1`), `MEISTER_SKIP_AGENT`.

Bind check: if `bind` is not a loopback address and `MEISTER_ALLOW_REMOTE != 1`, **refuse to start** with a non-zero exit (not a warning).

`GET /api/settings` reads `config/meister.json` (not SQLite) and returns slot names, model ids, and limits — never secrets.
...
You review a code change. Read .meister/prompt.md and follow it.
Write only the slot file named in the prompt (.meister/findings-model_a.json or findings-model_b.json). Never edit source files.
...
You judge candidates in .meister/judge-input.json (host ULIDs).
...
`miner.md` is the same permission skeleton with `.meister/mined-rules.json` allowed and a “emit rule objects, enabled=false” prompt.
...
- **Jobs (`/`):** a `$ meister status` stream. Each job is a block: title, SHA, status, pipeline line (`det → A ∥ B → judge`), then findings inline. Empty: “No jobs yet. In a repo run `meister review`.”
...
- **No upload.** Browser does not start, retry, or cancel a job (cancel stays CLI / API only unless we add a tiny `meister cancel` hint).

Polling every 2s while a job is non-terminal. Vite `/api` → `:8080`. Vapor `FileMiddleware` + SPA fallback.

### CLI (`Sources/MeisterCLI`, product `meister`)

Talks to `http://127.0.0.1:8080` (override `MEISTER_URL`).

| Command | What |
| --- | --- |
| `meister serve` | Start the API + SPA (or tell you it is already up) |
| `meister review [base-ref]` | Pack cwd, `POST /api/jobs` `scope=full`, print id and poll |
| `meister review --parent <job-id>` | Incremental pack + POST |
| `meister status [id]` | Latest job or one job |
| `meister cancel <id>` | `POST /api/jobs/:id/cancel` |
...
### SQLite schema (GRDB migrations, `var/meister.sqlite`)
...
**No `settings` table.** Runtime config is `config/meister.json` + env.
...
Zip is rejected. `pack-repo.sh` embeds `.meister/diff.patch` as the change-set and **may** embed a self-contained `.meister/history.bundle` (`git bundle create … "$BASE" "$HEAD"`, fetched with `git fetch`, never `A..B` into `git init`). A **bare** git bundle as the `archive` part (no working tree) is deferred.
...
| T8 | Bind `0.0.0.0` | Med | Refuse to start unless `MEISTER_ALLOW_REMOTE=1`. |
...
`swift-log` label `meister`. Every job event is also a `job_events` row. Include `job_id`. Do not log env, headers, or file contents at `info`.

### Metrics (`GET /api/metrics`, Prometheus text, localhost-only)

- `meister_jobs_total{status,scope}`
- `meister_job_duration_seconds{phase}`
- `meister_findings_total{phase,verdict,severity}`
- `meister_queue_depth` — **`SELECT COUNT(*) FROM jobs WHERE status='queued'`**, not a swift-jobs memory-driver API
- `meister_docker_oom_total`
- `meister_archive_bytes`
...
2. Flags: `limits.agent_timeout_sec`, `MEISTER_SKIP_AGENT=1`.
3. Pin `meister/opencode-runner:0.1.0`. Never `:latest`.
4. Rollback = previous Meister git SHA + previous image tag.
...
- Run: `swift run MeisterAPI serve --data-dir ./var --bind 127.0.0.1 --port 8080`
- Frontend dev: `cd frontend && npm run dev` (proxy `/api` → `:8080`)
- Runner: `scripts/build-runner.sh` → `meister/opencode-runner:0.1.0` (`--platform linux/arm64` on Apple Silicon)
...
- **Files / components:** `Package.swift` (`// swift-tools-version: 6.0`), Vapor `Sources/MeisterAPI/configure.swift` (`GET /api/health`, bind check, SPA fallback stub), `frontend/` Vite+React+TS, `docker/opencode-runner/Dockerfile` stub, `scripts/dev.sh`, `scripts/build-runner.sh`, `.gitignore`, `config/meister.example.json`
- **Depends on:** nothing
- **Description:** Runnable pair of processes. Health check. Refuse non-loopback bind without `MEISTER_ALLOW_REMOTE`.

### PR 2 — SQLite store, migrations, blob layout

- **Files / components:** `Sources/MeisterCore/Store/*`, GRDB actor, `v1_initial` (no `settings` table), `var/` gitignore, `GET /api/settings` from the JSON file
- **Depends on:** PR 1
- **Description:** Persistence exists.

### PR 3 — Safe tar extract + git change-set identification

- **Files / components:** `Sources/CLibArchive/*`, `ArchiveUnpacker.swift` (Darwin `chown` ignores `EPERM`/`ENOTSUP`; Linux requires success), `ChangeSet.swift` (prefer `.meister/diff.patch`; `git fetch` bundle, never unbundle / never `A..B`), `LanguageMap.swift`, `scripts/pack-repo.sh` (required `git diff` → `.meister/diff.patch`; optional self-contained bundle; fail if neither), fixtures (tiny-repo, Pax, abs symlink, hardlink, fifo, bomb), CryptoKit content SHA-256 tests
- **Depends on:** PR 2
...
- **Files / components:** `JobsRoute.swift`, `ReviewJob.swift`, `JobStateMachine.swift`, `BootReconcile.swift`, `WorkspaceGCJob.swift`, `Sources/MeisterCLI` (`meister review` / `status` / `cancel`), frontend Jobs list + polling (no upload)
- **Depends on:** PR 3
- **Description:** `meister review` with `MEISTER_SKIP_AGENT=1` walks `queued → succeeded`. Preferred pack with empty meta SHAs is **202**. GC and boot reconcile live.

### PR 5 — Rule schema, CRUD, seed rules, deterministic engine

- **Files / components:** `rules/*.yaml`, `PathGlob.swift`, dumb `RuleSelector` (enabled + glob + language only), `MeisterDeterministic/*` (Swift regex / deny_api / sibling_test), `RulesRoute.swift`, editor UI
- **Depends on:** PR 4
...
- **Files / components:** `agents/judge.md`, `JudgeMerge.swift`, `schemas/judge.json`, `schemas/judge-input.json`, numbered review→judge handoff (parse → ULID → `evidence_ok` → write `.meister/judge-input.json` → `--file` that), persist agent/pre/post blobs, UI “show dropped”
...
- **Files / components:** `MeisterMiner/*`, `CorpusRoute.swift`, `miner.md`, title/glob dedup (not BM25 > 8)
...
- **Description:** FTS ranking + embeddings table + cosine retrieve into `.meister/context.md`. Architecture index job. `/context` and `/learnings` UI. Learn-from-job fills the inbox.

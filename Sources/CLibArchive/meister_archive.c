...
void meister_archive_close(meister_archive *handle) {
    if (handle == NULL) {
        return;
    }
    if (handle->archive != NULL) {
...
int meister_archive_next_header(meister_archive *handle) {
...
const char *meister_archive_pathname(meister_archive *handle) {
...
const char *meister_archive_symlink(meister_archive *handle) {
    if (handle == NULL || handle->entry == NULL) {
        return NULL;
    }
...
const char *meister_archive_hardlink(meister_archive *handle) {
    if (handle == NULL || handle->entry == NULL) {
        return NULL;
    }
...
int64_t meister_archive_size(meister_archive *handle) {
    if (handle == NULL || handle->entry == NULL) {
        return 0;
    }
...
int meister_archive_size_is_set(meister_archive *handle) {
    if (handle == NULL || handle->entry == NULL) {
        return 0;
    }
...
int meister_archive_filetype(meister_archive *handle) {
    if (handle == NULL || handle->entry == NULL) {
        return 0;
    }
    unsigned int type = archive_entry_filetype(handle->entry);
    if (type == 0) {
        type = archive_entry_mode(handle->entry) & MEISTER_AE_IFMT;
    }
    return (int)type;
}

mode_t meister_archive_mode(meister_archive *handle) {
    if (handle == NULL || handle->entry == NULL) {
        return 0;
    }
...
const char *meister_archive_error_string(meister_archive *handle) {
    if (handle == NULL || handle->archive == NULL) {

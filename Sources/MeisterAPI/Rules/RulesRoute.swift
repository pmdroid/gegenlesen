...
enum RulesRoute {
    ...
    static func list(_ req: Request) async throws -> RuleListResponse {
        ...
        let rules = try await req.application.meisterStore.listRules(filter)
    ...
    static func create(_ req: Request) async throws -> Response {
        ...
        if try await req.application.meisterStore.rule(id: id) != nil {
            if upsert.id != nil {
                throw APIError.conflict("rule already exists")
            }
            id = try await uniqueID(base: id, store: req.application.meisterStore)
        }
        let rule = Rule(
            id: id,
            title: upsert.title,
    ...
    private static func requireRule(_ req: Request) async throws -> Rule {
        ...
        guard let rule = try await req.application.meisterStore.rule(id: RuleID(raw)) else {
            throw APIError.notFound()
        }
        return rule
    }

...
public struct ChangeSetIdentifier: Sendable {
    ...
    private func identifyEmbedded() throws -> ChangeSet? {
        let embedded = workspace.appendingPathComponent(".meister/diff.patch")
        ...
        let base = firstNonEmpty([
            meta.baseSHA,
            readMeisterFile("base_sha"),
            ContentHash.sha256(data),
        ]) ?? ContentHash.sha256(data)
        let head = firstNonEmpty([
            meta.headSHA,
            readMeisterFile("head_sha"),
            "noparent",
        ]) ?? "noparent"
        return ChangeSet(
            baseSHA: base,
    ...
    private func identifyMultipart(deadline: Date) throws -> ChangeSet? {
        ...
        let patchFile = workspace.appendingPathComponent(".meister/multipart.patch")
    ...
    private func resolveBase(git: GitRunner, head: String) throws -> String {
        if let sha = firstNonEmpty([meta.baseSHA, readMeisterFile("base_sha")]) {
            return sha
        }
        if let ref = meta.baseRef, let resolved = try? git.run(["rev-parse", ref]) {
            let trimmed = resolved.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        for candidate in ["main", "master", "trunk", "origin/HEAD"] {
            if let merge = try? git.run(["merge-base", candidate, head]) {
    ...
    private func readMeisterFile(_ name: String) -> String? {
        let url = workspace.appendingPathComponent(".meister/\(name)")

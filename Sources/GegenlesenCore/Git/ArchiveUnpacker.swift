...
public struct ArchiveUnpacker: Sendable {
    ...
    private func walk(archive: URL, gzip: Bool, write: Bool, workspace: URL) throws {
        let handle: OpaquePointer = try archive.path.withCString { path in
            guard let opened = gegenlesen_archive_open(path, gzip ? 1 : 0) else {
                throw ArchiveError.readFailed("unable to open archive")
            }
            return opened
        }
        defer { gegenlesen_archive_close(handle) }

        var fileCount = 0
        var uncompressed: Int64 = 0

        while true {
            let status = gegenlesen_archive_next_header(handle)
            if status == 0 {
                break
            }
            if status < 0 {
    ...
    private func processEntry(
        ...
        guard let rawPath = gegenlesen_archive_pathname(handle) else {
        ...
        let rawSize = gegenlesen_archive_size(handle)
        ...
        if let hardlink = gegenlesen_archive_hardlink(handle), hardlink.pointee != 0 {
            throw ArchiveError.hardlink
        }

        let mode = gegenlesen_archive_mode(handle)
        if mode & (S_ISUID | S_ISGID) != 0 {
            throw ArchiveError.setidBit
        }

        let type = gegenlesen_archive_filetype(handle)
        let kind: EntryKind
        switch type {
        case Int32(GEGENLESEN_AE_IFREG):
            kind = .file
        case Int32(GEGENLESEN_AE_IFDIR):
            kind = .directory
        case Int32(GEGENLESEN_AE_IFLNK):
            kind = .symlink
        default:
        ...
        if kind == .symlink {
            let target = gegenlesen_archive_symlink(handle).map { String(cString: $0) } ?? ""
    ...
    private func consumeData(
        ...
        while true {
            ...
            let status = withUnsafeMutablePointer(to: &buf) { bufPtr in
                gegenlesen_archive_read_data_block(handle, bufPtr, &size, &offset)
            }
            if status == 0 {
    ...
    private func cError(_ handle: OpaquePointer) -> String {
        if let message = gegenlesen_archive_error_string(handle) {

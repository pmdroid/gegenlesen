...
function parseArgs(argv) {
  const args = { timeoutSec: 900, message: "Investigate the change thoroughly, then write findings as instructed." };
  let i = 2;
  ...
  if (args.command.length === 0) {
    throw new Error("usage: spike-acp-client.mjs [--prompt-file path] [--prompt-uri uri] [--message text] [--transcript path] [--timeout-sec n] -- command args...");
  }
  return args;
}
...
function status(text) {
  process.stderr.write(`[acp-client] ${text}\n`);
}
...
function finish(code, reason) {
  ...
  setTimeout(() => process.exit(code), 7000).unref();
}
...
function handleIncoming(message) {
  ...
  if (message.id !== undefined) {
    send({ jsonrpc: "2.0", id: message.id, error: { code: -32601, message: `method not supported by spike client: ${message.method}` } });
  }
}
...
try {
  ...
  status(`initialized: protocolVersion=${init.protocolVersion} authMethods=${JSON.stringify(init.authMethods ?? [])}`);
  ...
  record({ dir: "client", event: "stop", stopReason: result.stopReason });

...
struct QuarantineTests {
    ...
    func copiesAndRenamesLoadableConfigLeavingAgents() throws {
        try withTempDir("quarantine") { root in
            ...
            #expect(fm.fileExists(atPath: root.appendingPathComponent("opencode.json.meister-disabled").path))
            #expect(fm.fileExists(atPath: root.appendingPathComponent(".opencode.meister-disabled/plugins/pwn.js").path))
            ...
            #expect(fm.fileExists(atPath: root.appendingPathComponent(".meister/quarantine/opencode.json").path))
            #expect(fm.fileExists(atPath: root.appendingPathComponent(".meister/quarantine/AGENTS.md").path))
            #expect(fm.fileExists(atPath: root.appendingPathComponent(".meister/quarantine/.opencode/plugins/pwn.js").path))
        }
    }

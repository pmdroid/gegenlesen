...
struct ChangeSetTests {
    ...
    func prefersEmbeddedDiffWithoutGit() throws {
        try withTempDir("meister-id-embedded") { dir in
            ...
            try writeFile(".meister/base_sha", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", in: workspace)
            try writeFile(".meister/head_sha", "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb", in: workspace)
    ...
    func identifiesFromGitWorkingTree() throws {
        try withTempDir("meister-id-git") { dir in
    ...
    func fetchesBundleInsteadOfUnbundle() throws {
        try withTempDir("meister-id-bundle") { dir in
            ...
            try git(["tag", "meister-base", base], cwd: repo)
            ...
            try writeFile(".meister/base_sha", base, in: workspace)
    ...
    func failsWithNoChangeSetWhenNothingIdentifies() throws {
        try withTempDir("meister-id-none") { dir in
            let workspace = dir.appendingPathComponent("ws")

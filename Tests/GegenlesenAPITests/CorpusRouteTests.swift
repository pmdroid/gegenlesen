...
struct CorpusRouteTests {
    ...
    func learnWithoutFeedbackDoesNotInboxEveryFinding() async throws {
        try await withGegenlesenApp(startQueue: true) { app in
            ...
            try await app.gegenlesenStore.insertFindings(
                [
                    FindingDraft(
                        ruleID: nil,
                        phase: .agent,
...
private func corpusTarGz() throws -> Data {
    let dir = FileManager.default.temporaryDirectory
        .appendingPathComponent("gegenlesen-corpus-\(UUID().uuidString)", isDirectory: true)
    ...
    let archive = FileManager.default.temporaryDirectory
        .appendingPathComponent("gegenlesen-corpus-\(UUID().uuidString).tar.gz")
    let process = Process()

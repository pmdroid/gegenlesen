...
struct BindPolicyTests {
    ...
    func allowRemoteEnvIsStrictOne() {
        #expect(BindPolicy.allowRemoteFromEnvironment(["GEGENLESEN_ALLOW_REMOTE": "1"]))
        #expect(!BindPolicy.allowRemoteFromEnvironment(["GEGENLESEN_ALLOW_REMOTE": "true"]))
        #expect(!BindPolicy.allowRemoteFromEnvironment(["GEGENLESEN_ALLOW_REMOTE": "0"]))

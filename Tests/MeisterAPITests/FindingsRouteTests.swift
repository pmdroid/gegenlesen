...
struct FindingsRouteTests {
    ...
    func eventsAndTranscript() async throws {
        try await withMeisterApp { app in
            let finding = try await seedFinding(app)
            try await app.meisterStore.appendEvent(jobID: finding.jobID, level: .info, message: "reviewing")
            ...
            let url = app.meisterStore.blobs.transcriptURL(jobID: finding.jobID.rawValue, phase: "review")
...
private func seedFinding(_ app: Application, filePath: String? = "Sources/Auth/Session.swift") async throws -> Finding {
    ...
    try await app.meisterStore.insertParsedFindings([finding])
    return finding
}
